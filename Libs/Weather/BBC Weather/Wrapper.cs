...
namespace JB.Weather.BBC_Weather {
    internal class Wrapper : IWrapper {
        public async Task<JB.Common.IReturnCode<Interfaces.IForcast>> GetTodaysForcast(string pAreaCode) {
        ...
        public async Task<JB.Common.IReturnCode<IList<Interfaces.IForcast>>> Get3DayForcast(string pAreaCode) {
            JB.Common.IReturnCode<IList<Interfaces.IForcast>> rc = new JB.Common.ReturnCode<IList<Interfaces.IForcast>>();
            ...
            if (JB.Common.ErrorCodes.SUCCESS == rc.ErrorCode) {
                ...
                if (HttpStatusCode.OK != response.StatusCode) {
                    rc.ErrorCode = 3;
                    JB.Common.ErrorWorker.AddError(rc, rc.ErrorCode);
                }
